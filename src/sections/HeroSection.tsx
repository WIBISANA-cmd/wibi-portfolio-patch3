...
export default function HeroSection({ data, ready = true }: { data: HeroData; ready?: boolean }) {
  ...
  useGSAP(
    () => {
      ...
      mm.add('(prefers-reduced-motion: no-preference)', () => {
        const textTargets = [eyebrowRef.current, headlineRef.current, subRef.current, ctaRef.current].filter(Boolean);

        // Hidden starting state — held until the preloader hands off.
        gsap.set(textTargets, { opacity: 0, y: 26 });
        gsap.set(imageRef.current, { opacity: 0, y: 20, scale: 1.06 });

        if (!ready) return;

        const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
        tl.to(eyebrowRef.current, { opacity: 1, y: 0, duration: 0.8 })
          .to(headlineRef.current, { opacity: 1, y: 0, duration: 1 }, '-=0.55')
          .to(subRef.current, { opacity: 1, y: 0, duration: 0.8 }, '-=0.6')
          .to(ctaRef.current, { opacity: 1, y: 0, duration: 0.7 }, '-=0.5')
          .to(imageRef.current, { opacity: 1, y: 0, scale: 1, duration: 1.3, ease: 'power2.out' }, '-=0.9');
      });

      mm.add('(prefers-reduced-motion: reduce)', () => {
        gsap.set([eyebrowRef.current, headlineRef.current, subRef.current, ctaRef.current, imageRef.current], {
          opacity: 1,
          y: 0,
          scale: 1,
        });
  ...
  return (
    <section
      ref={sectionRef}
      className="bg-gray-50 flex flex-col font-sans text-gray-900 overflow-hidden pt-24 min-h-[calc(100vh-80px)]"
    >
      {/* Main Content Area */}
      <div className="flex-grow flex flex-col lg:flex-row items-center justify-between px-6 md:px-8 lg:px-16 py-8 md:py-12 max-w-[1440px] mx-auto w-full gap-12 lg:gap-8">
        ...
        <div className="w-full lg:w-[45%] flex flex-col items-start space-y-6 md:space-y-8 z-10 pt-4 lg:pt-0" data-speed="1.1">
          ...
          {eyebrow && (
            <div ref={eyebrowRef} className="inline-flex items-center bg-[#F3F4F0] border border-gray-200 rounded-full py-1.5 px-3 md:py-2 md:px-4 shadow-sm" data-lag="0.2">
          ...
          {/* Main Headline */}
          <h1 ref={headlineRef} className="text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] xl:text-[6.5rem] leading-[1.05] font-serif text-[#1A1A1A] tracking-tight whitespace-pre-line">
            {heading}
          </h1>

          {/* Subheadline */}
          {subheadline && (
            <p ref={subRef} className="text-base sm:text-lg md:text-xl text-gray-700 max-w-md md:max-w-lg font-normal leading-relaxed">
              {subheadline}
            </p>
          )}
          ...
          {/* CTA Button */}
          {ctaLabel && ctaHref && (
            <a ref={ctaRef} href={ctaHref} className="bg-[#2A2A2A] text-white px-6 md:px-8 py-3 md:py-3.5 rounded-lg text-sm md:text-base font-medium hover:bg-black transition-colors shadow-md mt-2 md:mt-4">
              {ctaLabel}
            </a>
          )}
        </div>
        ...
        <div className="w-full lg:w-[55%] relative flex justify-center lg:justify-end mt-8 lg:mt-0 pb-8 lg:pb-0" data-speed="0.95">
          ...
          <div ref={imageRef} className="relative w-full max-w-[500px] lg:max-w-[700px] aspect-[4/3] md:aspect-[16/11] lg:aspect-[4/3] xl:aspect-[16/11]">
